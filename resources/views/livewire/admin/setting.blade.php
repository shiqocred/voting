<div class="bg-[url('../img/hero-60.png')] min-h-screen bg-slate-400">
    ...
    <div class="w-screen  flex justify-center pt-[130px] px-5 pb-[50px] md:px-0">
        <div class="w-full sm:w-3/5 lg:w-1/3 h-[600px] bg-slate-300 relative  rounded-md flex flex-col items-center justify-center space-y-5">
            @if (session()->has('success'))
                <div class="flex w-full h-12 mb-3 justify-center items-center">
                    <div class="w-10/12 md:w-3/4 h-full bg-green-500 text-white px-10 flex items-center justify-start rounded-md">
                        <x-svg-check  width="18px" height="18px"/>
                        &nbsp; &nbsp;
                        <h3 class="text-sm capitalize">{{ session('success') }}Setting update Successfully</h3>
                    </div>
                </div>
            @endif
