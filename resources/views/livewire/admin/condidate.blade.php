                    ...
                    @if (count($condidates) > 0)
                        @foreach ($condidates as $condidate)    
                            <tr class="odd:bg-white even:bg-slate-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap flex items-center text-sm font-medium text-slate-900">
                                    <input type="color" class="w-10" value="{{ $condidate->color }}" disabled />
                                    {{ $condidate->color }}
                                </td> 
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                                    <img src="/storage/{{ $condidate->image }}" class="h-10 border rounded-md" alt="">
                                </td> 
                                <td class="px-2 w-[200px] py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                                    <button 
                                        class="bg-green-500 hover:bg-green-600 focus:outline-none focus:ring focus:ring-green-300 active:bg-green-700 px-3 py-1 mr-3 text-sm leading-5 rounded-sm font-semibold text-white"
                    ...
                    <div class="w-11/12 sm:w-[48%] py-7 bg-slate-700/50 backdrop-blur-md lg:hidden rounded-md mt-4 flex space-y-5 flex-col items-center justify-center">
                        ...
                        <div class="w-4/5 flex flex-col justify-center space-y-5">
                            <div  class="text-white min-w-full flex flex-col text-start items-start justify-center space-y-3">
                                ...
                                <div class="flex w-full flex-col ">
                                    <span class="font-semibold">Sebagai: </span>
                                ...
                                <div class="flex w-full flex-col">
                                    ...
                                    <div class="flex w-full">
                                        <input type="color" class="border-0 bg-slate-300 rounded-sm px-3 ring-0 w-1/2 mr-5" value="{{ $condidate->color }}" disabled />
                                        {{ $condidate->color }}
                                    </div>
                                </div>
                            </div>
        ...
        <div class="w-screen flex flex-col items-center mb-5">
            <form wire:submit.prevent='store' class="w-11/12 md:w-4/5 flex flex-col justify-center">
                ...
                <label class="block w-full mt-5">
                    ...
                    @enderror
                    @if ($image)
                        <img src="{{ $image->temporaryUrl() }}" class="w-full md:w-3/5 lg:w-2/5 mt-3 rounded-md border" alt="">
                    @endif
                </label>
    ...
    <div class="w-screen flex flex-col items-center mb-5">
        <form wire:submit.prevent='update({{ $condidate_id }})' class="w-11/12 md:w-4/5 flex flex-col justify-center">
            ...
            <label class="block w-full mt-5">
                ...
                />
                @if ($new_image)
                    <img src="{{ $new_image->temporaryUrl() }}" class="w-full md:w-3/5 lg:w-2/5 mt-3 rounded-md border" alt="">
                    @else
                    <img src="/storage/{{ $old_image }}" class="w-full md:w-3/5 lg:w-2/5 mt-3 rounded-md border" alt="">
                @endif
            </label>
            <div class="flex mt-5 space-x-3">
                <button type="button" class="bg-slate-500 hover:bg-slate-600 focus:outline-none focus:ring focus:ring-slate-300 active:bg-slate-700 w-10 h-10 mb-5 text-sm leading-5 rounded-md font-semibold text-white flex items-center justify-center" wire:click='goBack'>
